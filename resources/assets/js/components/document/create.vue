					...
					<table class="table">
						<tbody>
							<tr v-for="(row, index) in subDocuments">
								<td>
									<input type="text" class="code form-control form-control-sm border-secondary" v-on:keyup.enter="addSubDocuments(row,index)" v-model="row.code" v-focus>
								</td>
								<td>
									<input type="text" class="form-control form-control-sm" v-model="row.title" readonly>
								</td>
								<td>
			...
			<div class="col-md-6 order-md-1">
				...
				<div class="form-group">
					<select v-model="list.document_type_id" class="form-control border-secondary">
						...
						<option v-for="(value,key) in document_type" :value="value.id" >
							{{ value.document_type }}
						</option>
					</select>
				</div>
	...
	export default{
		...
		methods:{
			...
			save(){		
			    ...
			    axios.post('/documents',[this.list, this.subDocuments, this.process]).then((response)=> 
						{
							this.$message({
								type: 'success',
